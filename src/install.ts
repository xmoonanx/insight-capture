...
function parseOnlyArg(args: string[]): SkillTarget | undefined {
...
function getLocations(only?: SkillTarget): SkillLocation[] {
  const home = homedir();
...
async function removeExisting(location: SkillLocation, options: InstallOptions): Promise<boolean> {
...
function showHelp(): void {
  console.log(
    'Usage: insight-install [--dry-run] [--force] [--verbose] [--only=claude|codex] [--uninstall] [--from-postinstall]'
  );
}
